...
//串口发送缓存区 	
__align(8) u8 USART4_TX_BUF[USART4_MAX_SEND_LEN]; 	//发送缓冲,最大USART2_MAX_SEND_LEN字节
...
//USART4_TX   PC10
 ...
 //	USART2->BRR=(pclk1*1000000)/(bound);// 波特率设置	 
	...
	UART_DMA_Config(DMA2_Channel5,(u32)&UART4->DR,(u32)USART4_TX_BUF);//DMA1通道7,外设为串口4,存储器为USART4_TX_BUF 
...
{  
	...
	va_end(ap);
	while(DMA_GetCurrDataCounter(DMA2_Channel5)!=0);	//等待通道7传输完成   
	UART_DMA_Enable(DMA2_Channel5,strlen((const char*)USART4_TX_BUF)); 	//通过dma发送出去
}
//定时器4中断服务程序		    
